...
import { STATIONS, EVENTS, forecast7d, riskMeta } from '../data/mockData';

export default function ForecastView() {
  const { patch, showToast } = useApp();

  const fcRows = STATIONS.map((s) => {
    ...
    const spark = f.map((d) => ({ color: d.color, h: `${3 + d.val * 2.4}px` }));
    const meta = riskMeta(s.riskStatus);
    return { ...s, score, spark, peakDay: f[pi].day, peakVal: peak.val, riskColor: meta.color, riskLabel: meta.label };
  }).sort((a, b) => b.score - a.score);

  const fcHigh = STATIONS.filter((s) => (s.riskScore ?? 0) >= 5).length;
  const fcVeryHigh = STATIONS.filter((s) => (s.riskScore ?? 0) >= 7).length;
      ...
      <div style={{ maxWidth: 1260, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
          <div style={{ background: '#fff', border: '1px solid #E8EAEE', borderRadius: 14, padding: '16px 18px' }}>
            ...
            <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "'IBM Plex Mono',monospace", marginTop: 4 }}>{STATIONS.length}</div>
          ...
          <div style={{ background: '#fff', border: '1px solid #E8EAEE', borderRadius: 14, padding: '16px 18px', borderLeft: '3px solid #F97316' }}>
            ...
            <div style={{ fontSize: 11.5, color: '#9AA0A6', marginTop: 2 }}>Chỉ số ≥ 5 trong 5–7 ngày</div>
          </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #E8EAEE', borderRadius: 14, overflow: 'hidden' }}>
          ...
          {fcRows.map((r) => (
            ...
            >
              ...
              <span style={{ fontSize: 12.5, color: '#6B7280' }}>{r.province?.name ?? '—'}</span>
